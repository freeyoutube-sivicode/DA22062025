...
const AdminEditProductPage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        toast.error('Lỗi khi tải dữ liệu sản phẩm: ' + (err.response?.data?.message || err.message));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (formData.Stock !== undefined) data.append('Stock', formData.Stock.toString());
      ...
      if (formData.Specifications) data.append('Specifications', JSON.stringify(formData.Specifications));
      ...
      const updatedProduct = await productService.updateProduct(id, data as any); // Cast to any for FormData
  ...
  return (
    <div className="admin-edit-product-page">
      ...
      <form onSubmit={handleSubmit} className="bg-white p-6 rounded-lg shadow-md">
         {/* Product Name */}
        <div className="mb-4">
          <label htmlFor="Product_Name" className="block text-gray-700 font-bold mb-2">Tên sản phẩm</label>
          <input
            type="text"
            id="Product_Name"
            name="Product_Name"
        ...
        {/* Category */}
        <div className="mb-4">
          <label htmlFor="CategoryID" className="block text-gray-700 font-bold mb-2">Danh mục</label>
          <select
            id="CategoryID"
            name="CategoryID"
        ...
        {/* Main Image */}
        <div className="mb-4">
          <label htmlFor="Main_Image" className="block text-gray-700 font-bold mb-2">Ảnh chính</label>
          {/* Display existing image if available */}
          {typeof formData.Main_Image === 'string' && formData.Main_Image && (
             <div className="mb-2">
                <img src={formData.Main_Image} alt="Existing Main" className="w-32 h-32 object-cover rounded"/>
             </div>
          )}
          <input
            type="file"
        ...
        {/* List Images */}
        <div className="mb-4">
          <label htmlFor="List_Image" className="block text-gray-700 font-bold mb-2">Ảnh phụ</label>
          {/* Display existing images with remove option */}
          {existingImages.length > 0 && (
             <div className="flex flex-wrap gap-2 mb-2">
                {existingImages.map((image, index) => (
                   <div key={index} className="relative">
                      <img src={image} alt={`Existing ${index + 1}`} className="w-24 h-24 object-cover rounded"/>
                      <button type="button" onClick={() => handleRemoveExistingImage(image)} className="absolute top-0 right-0 bg-red-500 text-white rounded-full p-1 text-xs leading-none">
                         &times;
                      </button>
                   </div>
                ))}
         ...
         <div className="mb-4">
           ...
           {/* Example static fields - replace with dynamic input fields later */}
            <div className="grid grid-cols-2 gap-4">
               <div>
                  <label htmlFor="spec-engine" className="block text-gray-700 text-sm font-bold mb-2">Engine</label>
                   <input type="text" id="spec-engine" value={formData.Specifications?.Engine || ''} onChange={(e) => handleSpecificationChange('Engine', e.target.value)} className="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"/>
               </div>
               <div>
                  <label htmlFor="spec-hp" className="block text-gray-700 text-sm font-bold mb-2">Horsepower</label>
                   <input type="text" id="spec-hp" value={formData.Specifications?.Horsepower || ''} onChange={(e) => handleSpecificationChange('Horsepower', e.target.value)} className="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"/>
               </div>
        ...
        <div className="flex items-center justify-between">
          ...
          <Link to="/admin/products" className="inline-block align-baseline font-bold text-sm text-blue-500 hover:text-blue-800">
            Hủy
          </Link>
        </div>
      </form>
